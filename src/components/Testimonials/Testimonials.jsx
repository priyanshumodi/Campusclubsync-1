import React from "react";
import Slider from "react-slick" ; // jai 

const TestimonialData = [
  {
    id: 1,
    name: "Sarika purohit (GDSC LEAD)",
    text: "The role of a GDSC (Google Developer Student Clubs) lead in college is similar to that of a GFG (GeeksforGeeks) lead in terms of organizing technical events, workshops, hackathons, coding competitions, and fostering a culture of learning and collaboration among students. However, GDSC leads specifically focus on Google technologies, platforms, and tools, aligning their activities with Google's initiatives and resources for student developers. They also work closely with Google mentors and industry experts to provide guidance and support to students interested in Google technologies and career paths.",
    img: "https://res.cloudinary.com/startup-grind/image/upload/c_fill,dpr_2.0,f_auto,g_center,q_auto:good/v1/gcs/platform-data-dsc/events/Sarika%20Purohit%20Profile.jpg",
  },
  
  {
    id: 3,
    name: "sarthak joleya (GfG)",
    text: "A GFG lead in college is responsible for organizing coding contests, workshops, and seminars, promoting a coding culture, facilitating learning opportunities, building a community of tech enthusiasts, collaborating with industry, managing resources, and providing support and guidance to students in improving their coding skills and advancing their careers. reiciendis inventore iste ratione ex alias quis magni at optio",
    img: "https://media.licdn.com/dms/image/D4D03AQENJGd9ChdVMA/profile-displayphoto-shrink_400_400/0/1666810078918?e=2147483647&v=beta&t=YkMGCFYY6JxajSwLsMGPU98kdFV-lDIcqZETQCfXJkY",
  },
  {
    id: 2,
    name: "Prabal Patidar (ex GDSC LEAD)",
    text: "The role of a GDSC (Google Developer Student Clubs) lead in college is similar to that of a GFG (GeeksforGeeks) lead in terms of organizing technical events, workshops, hackathons, coding competitions, and fostering a culture of learning and collaboration among students. However, GDSC leads specifically focus on Google technologies, platforms, and tools, aligning their activities with Google's initiatives and resources for student developers. They also work closely with Google mentors and industry experts to provide guidance and support to students interested in Google technologies and career paths.",
    img: "data:image/jpeg;base64,",
  },
  {
    id: 5,
    name: "Priyanshu Modi (Algozenith)",
    text: "AlgoZenith is a platform or organization that focuses on algorithmic programming and competitive coding. A role similar to a GDSC or GFG lead in AlgoZenith would involve organizing algorithmic coding contests, workshops on data structures and algorithms, promoting a culture of competitive programming, facilitating learning opportunities related to algorithms and problem-solving techniques, building a community of algorithm enthusiasts, collaborating with industry partners for mentorship and career opportunities, managing resources for coding practice and skill development, and providing support and guidance to students in improving their algorithmic skills and preparing for competitive coding competitions.",
    img: "https://lh3.googleusercontent.com/a/AGNmyxZ0gxQdXxZo9v0evHJWvmTJqjmFdpBlt3EVC0RxQA=w60-h60",
  },
  {
    id: 6,
    name: "Muskan kushwah(Mern Stack developer)",
    text: "A role similar to a GDSC, GFG, or AlgoZenith lead in the context of a MERN (MongoDB, Express.js, React.js, Node.js) developer would involve organizing events, workshops, and seminars focused on MERN stack development. The responsibilities would include promoting a culture of full-stack development using the MERN stack, facilitating learning opportunities in web development, building a community of MERN stack enthusiasts, collaborating with industry partners for MERN stack projects and internships, managing resources for MERN stack education and training, providing support and guidance to students in developing their MERN stack skills, and preparing them for careers as MERN stack developers or full-stack developers.",
    img: "https://avatars.githubusercontent.com/u/131872972?s=48&v=4",
  },
  {
    id: 7,
    name: "Himanshi dubey(Data Scientist)",
    text: "A role similar to a GDSC, GFG, or AlgoZenith lead in the field of data science would involve organizing events, workshops, and seminars focused on data science topics such as machine learning, data analysis, data visualization, and big data technologies. The responsibilities would include promoting a data-driven culture, facilitating learning opportunities in data science, building a community of data enthusiasts, collaborating with industry partners for data science projects and internships, managing resources for data science education and training, providing support and guidance to students in developing their data science skills, and preparing them for careers in data science and related fields.",
    img: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTZ8fHByb2Zlc3Npb25hbCUyMGdpcmx8ZW58MHx8MHx8fDA%3D",
  }
];

const Testimonials = () => {
  var settings = {
    dots: true,
    arrows: false,
    infinite: true,
    speed: 500,
    slidesToScroll: 1,
    autoplay: true,
    autoplaySpeed: 2000,
    cssEase: "linear",
    pauseOnHover: true,
    pauseOnFocus: true,
    responsive: [
      {
        breakpoint: 10000,
        settings: {
          slidesToShow: 3,
          slidesToScroll: 1,
          infinite: true,
        },
      },
      {
        breakpoint: 1024,
        settings: {
          slidesToShow: 2,
          slidesToScroll: 1,
          initialSlide: 2,
        },
      },
      {
        breakpoint: 640,
        settings: {
          slidesToShow: 1,
          slidesToScroll: 1,
        },
      },
    ],
  };

  return (
    <div className="py-10 mb-10">
      <div className="container">
        {/* header section */}
        <div className="text-center mb-10 max-w-[600px] mx-auto">
          <p data-aos="fade-up" className="text-sm text-primary">
            What our club members are saying
          </p>
          <h1 data-aos="fade-up" className="text-3xl font-bold">
            Testimonials
          </h1>
          <p data-aos="fade-up" className="text-xs text-white">
            Meet with our past event manager students
          </p>
        </div>

        {/* Testimonial cards */}
        <div data-aos="zoom-in">
          <Slider {...settings}>
            {TestimonialData.map((data) => (
              <div className="my-6">
                <div
                  key={data.id}
                  className="flex flex-col gap-4 shadow-lg py-8 px-6 mx-4 rounded-xl dark:bg-gray-800 bg-primary/10 relative"
                >
                  <div className="mb-4">
                    <img
                      src={data.img}
                      alt=""
                      className="rounded-full w-20 h-20"
                    />
                  </div>
                  <div className="flex flex-col items-center gap-4">
                    <div className="space-y-3">
                      <p className="text-xs text-gray-500">{data.text}</p>
                      <h1 className="text-xl font-bold text-black/80 dark:text-light">
                        {data.name}
                      </h1>
                    </div>
                  </div>
                  <p className="text-black/20 text-9xl font-serif absolute top-0 right-0">
                    ,,
                  </p>
                </div>
              </div>
            ))}
          </Slider>
        </div>
      </div>
    </div>
  );
};

export default Testimonials;
